...
and there are many repositories all implemented as traits that use the args to implement the interfaces.
These repositories often reference each other using the cake pattern

In many cases, at the overall domain library level an abstract interface for data will have two implementations:
